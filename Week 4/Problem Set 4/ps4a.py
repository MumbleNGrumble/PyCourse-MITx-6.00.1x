...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:    # As long as there are still letters left in the hand:
        ...
        if word == ".":    # If the input is a single period:
            ...
            if not isValidWord(word, hand, wordList):    # If the word is not valid:
                ...
                wordScore = getWordScore(word, n)    # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
    ...
    if calculateHandlen(hand) == 0:    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
...
def playGame(wordList):
    ...
    while True:
        playInput = str(input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")).lower()
